...
class EchoBackend(ModelBackend):
    ...
    EXTRACTION_KEYWORDS = {
        ...
        "golden retriever": ("semantic", "User's dog's name is Max, a golden retriever"),
        ...
        "japan": ("semantic", "User is planning a trip to Japan with a budget of $5000"),
        ...
        "mit": ("semantic", "User graduated from MIT in 2020 with Computer Science degree"),
        "college": ("semantic", "User graduated from MIT in 2020 with Computer Science degree"),
        "university": ("semantic", "User graduated from MIT in 2020 with Computer Science degree"),
        "graduated": ("semantic", "User graduated from MIT in 2020 with Computer Science degree"),
        "computer science": ("semantic", "User graduated from MIT in 2020 with Computer Science degree"),
        "italian place": ("semantic", "User's favorite restaurant is The Italian Place on 5th Avenue"),
        "5th avenue": ("semantic", "User's favorite restaurant is The Italian Place on 5th Avenue"),
        ...
        "home address": ("semantic", "User's home address is 123 Main Street, New York"),
        ...
        "wedding": ("semantic", "User's wedding is on June 15th, 2026 with fiancée Emily"),
    ...
    def _extract_facts_from_messages(self, messages: List[Dict[str, str]]) -> List[Dict]:
        ...
        logger.info(f"🧠 Smart extraction: found {len(facts)} facts from {len([m for m in messages if m.get('role') == 'user'])} user messages")
        return facts
    ...
    def _rewrite_query(self, messages: List[Dict[str, str]]) -> str:
        ...
        if len(query.split()) < 6 or any(word in query.lower() for word in ["it", "that", "this", "my"]):
        ...
        match = re.match(r"(?:Where did I (?:go to|attend) )(.+?)[\?\.]*$", query, re.IGNORECASE)
        if match:
            topic = match.group(1).strip()
            if "college" in topic.lower() or "university" in topic.lower() or "school" in topic.lower():
        ...
        match = re.match(r"(?:Where does (?:she|he|my friend|sarah) live)[\?\.]*$", query, re.IGNORECASE)
        if match:
        ...
        match = re.match(r"(?:How much (?:can I|do I|should I) (?:spend|budget|afford)).*[\?\.]*$", query, re.IGNORECASE)
        if match:
        ...
        match = re.match(r"(?:How many )(.+?)(?:are we|do we|will we|expecting).*[\?\.]*$", query, re.IGNORECASE)
        if match:
            topic = match.group(1).strip()
