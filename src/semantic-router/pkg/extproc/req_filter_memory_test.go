...
// Test-local mock types for constructing OpenAI-compatible JSON in mock servers.
...
func TestFormatMemoriesAsContext_SingleMemory(t *testing.T) {
	...
	assert.Contains(t, result, "relevant context from previous conversations")
...
// =============================================================================
// Memory Opt-Out Tests (SDK-managed memory prevention)
// =============================================================================
...
func TestMemoryOptOut_NoDisabledRoutes(t *testing.T) {
	router := &OpenAIRouter{
		Config: &config.RouterConfig{
			Memory: config.MemoryConfig{
				Enabled:        true,
				DisabledRoutes: nil, // No disabled routes
			},
		},
	}
...
func TestMemoryOptOut_NoDisabledModels(t *testing.T) {
	router := &OpenAIRouter{
		Config: &config.RouterConfig{
			Memory: config.MemoryConfig{
				Enabled:        true,
				DisabledModels: []string{}, // Empty list
			},
		},
	}
