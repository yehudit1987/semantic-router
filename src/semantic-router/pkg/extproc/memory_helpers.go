...
func extractMemoryInfo(ctx *RequestContext) (sessionID string, userID string, history []memory.Message, err error) {
	// Extract userID first (required for memory extraction)
	...
	if ctx.ResponseAPICtx != nil && ctx.ResponseAPICtx.IsResponseAPIRequest {
		...
		// Not a Response API request - cannot track turns without ConversationID
		// Return error to prevent turnCounts from leaking
