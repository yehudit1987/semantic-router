...
const (
	...
	timeoutComponentDeploy      = 2 * time.Minute // For Envoy Gateway, AIBrix core, Demo LLM
...
func (p *Profile) deployAIBrixCore(ctx context.Context, opts *framework.SetupOptions) error {
	...
	for _, dep := range deployments {
		p.log("Waiting for %s/%s to be ready...", dep.namespace, dep.name)
		if err := p.waitForDeployment(ctx, opts, dep.namespace, dep.name, timeoutComponentDeploy); err != nil {
