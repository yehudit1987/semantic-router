...
func (f *ResponseAPIFilter) toStoredResponse(req *responseapi.ResponseAPIRequest, resp *responseapi.ResponseAPIResponse) *responseapi.StoredResponse {
	// Parse input items - handle both string and array formats
	...
	if err := json.Unmarshal(req.Input, &inputItems); err != nil {
		...
		var inputStr string
		if err := json.Unmarshal(req.Input, &inputStr); err == nil && inputStr != "" {
			// Convert string to InputItem
			contentJSON, _ := json.Marshal(inputStr)
			inputItems = []responseapi.InputItem{
				{
					Type:    "message",
					Role:    "user",
					Content: contentJSON,
